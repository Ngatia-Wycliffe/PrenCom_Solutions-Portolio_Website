<?php include 'includes/header.php'; ?>
			...
			<div class="view-header d-flex justify-content-between pl-3 pt-0 pb-0 pr-2">
				<nav class="nav">
					...
					<a class="nav-link ck-active" href="#">Submitted</a>
			  		...
			  		<table class="table mytable">
			  			...
			  			foreach($tasks as $task) :?>
			  				<tr id="task-<?php echo $task['task_id'];?>">
			  					...
			  					<td style="font-weight: bold;"><?php $time = strtotime($task['submitted_on']);
			  							  $submitted_on = date('jS M Y', $time);
			  							  echo $submitted_on;
			  						?></td>
			  					<td><button onclick="approveTask(<?php echo $task['task_id'];?>)" class="btn-sm btn-default ck-btn" style="border: none">Approve</button>
			  					<button onclick="reSend(<?php echo $task['task_id'];?>)" class="btn-sm btn-default ck-btn" style="border: none" id="deleteAssigned"  >Resend</button>
			  					</td>
			  				</tr>
			  			<?php endforeach; ?>
			  			</tbody>
		...
		<script>
				function approveTask (task) {
					$.post("approveTask.php", {taskId: task}, function (data) {
					/* body... */
					alert(data);
		...
		function gotoDetails(task){
				window.location.href = 'taskdetails.php?id='+task;
		}
		</script>
